...
droplets_data = pd.read_csv(r"L:\29092025_BSF_densyl polymyxicin B\A2\droplets data.csv")
counts_data = pd.read_csv(r"L:\29092025_BSF_densyl polymyxicin B\A2\results_bg_corrected_per_droplet.csv")
GFP_data=pd.read_csv(r"L:\29092025_BSF_densyl polymyxicin B\A2\GFP_intesity_analysis_BG_corrected_per_droplet.csv")
...
window = 3000
GFP_sorted = t5_subset_DAPI.sort_values('Volume').reset_index(drop=True)
GFP_sorted['signal_MA'] = GFP_sorted['Mean_Top25_Intensity'].rolling(window=window, min_periods=10).mean()
mCherry_sorted = t5_subset_DAPI.sort_values('Volume').reset_index(drop=True)
mCherry_sorted['bac_MA'] = mCherry_sorted['GFP intesity'].rolling(window=window, min_periods=10).mean()
color1 = 'blue'
ax1.set_xlabel(r'Volume (µm$^3$)', fontsize=18)
ax1.set_ylabel('DAPI intensity per bacterium', fontsize=18, color=color1)
line1 = ax1.plot(GFP_sorted['Volume'], GFP_sorted['signal_MA'], color=color1, linewidth=1, label=f'DAPI {window}-point MA')
...
line2 = ax2.plot(mCherry_sorted['Volume'], mCherry_sorted['bac_MA'], color=color2, linewidth=1, label=f'GFP {window}-point MA')
...
GFP_sorted['signal_MA'] = GFP_sorted['Mean_Top25_Intensity'].rolling(window=20, min_periods=10).mean()
mCherry_sorted = t5_per_droplet_GFP.sort_values('Volume').reset_index(drop=True)
mCherry_sorted['bac_MA'] = mCherry_sorted['GFP intesity'].rolling(window=100, min_periods=10).mean()
color1 = 'blue'
ax1.set_xlabel(r'Volume (µm$^3$)', fontsize=18)
ax1.set_ylabel('DAPI intensity per droplet', fontsize=18, color=color1)
line1 = ax1.plot(GFP_sorted['Volume'], GFP_sorted['signal_MA'], color=color1, linewidth=1, label='DAPI 20-point MA')
ax1.tick_params(axis='y', labelcolor=color1)
ax1.set_xscale('log')
ax1.set_ylim(0,200)
ax1.set_xlim(1e3,1e8)
...
line2 = ax2.plot(mCherry_sorted['Volume'], mCherry_sorted['bac_MA'], color=color2, linewidth=1, label='GFP 100-point MA')
ax2.tick_params(axis='y', labelcolor=color2)
ax2.set_ylim(0, 2500)
ax2.set_xlim(1e3, 1e8)
lines = line1 + line2
...
# plt.scatter(t5_subset_GFP['Volume'], t5_subset_GFP['GFP intesity'],s=5, alpha=0.5)
# window = 3000
# t5_sorted = t5_subset_GFP.sort_values('Volume').reset_index(drop=True)
# t5_sorted['GFP_MA'] = t5_sorted['GFP intesity'].rolling(window=window, min_periods=1).mean()
# plt.plot(t5_sorted['Volume'], t5_sorted['GFP_MA'], color='red', linewidth=1, label=f'{window}-point MA')
# plt.xlabel('Volume')
# plt.xscale('log')
# plt.ylabel('GFP Intensity')
# plt.yscale('log')
# plt.title('GFP Intensity vs Volume per Bacterium T=5')
# plt.legend()
# plt.show()
#
# # --------------------------------------------------------------------------------------
...
# # --------------------------------------------------------------------------------------
#
rho, p_val = spearmanr(np.log10(t5_subset_DAPI['Volume']), t5_subset_DAPI['Mean_Top25_Intensity'])
plt.figure(figsize = (10,6))
plt.scatter(t5_subset_DAPI['Volume'], t5_subset_DAPI['Mean_Top25_Intensity'], s=1, alpha=0.3, c='grey')
window = 3000
t5_sorted = t5_subset_DAPI.sort_values('Volume').reset_index(drop=True)
t5_sorted['DAPI_MA'] = t5_sorted['Mean_Top25_Intensity'].rolling(window=window, min_periods=1).mean()
plt.plot(t5_sorted['Volume'], t5_sorted['DAPI_MA'], color='blue', linewidth=1,
...
# # --------------------------------------------------------------------------------------
rho, p_val = spearmanr(np.log10(t5_per_droplet_DAPI['Volume']), t5_per_droplet_DAPI['Mean_Top25_Intensity'])
plt.figure(figsize = (10,6))
plt.scatter(t5_per_droplet_DAPI['Volume'], t5_per_droplet_DAPI['Mean_Top25_Intensity'], s=5, alpha=0.5, c='grey')
window = 20
...
# plt.plot(t22_sorted['Volume'], t22_sorted['GFP_MA'], color='red', linewidth=1, label=f'{window}-point MA')
# plt.xlabel('Volume')
# plt.xscale('log')
# plt.ylabel('GFP Intensity')
# plt.yscale('log')
...
# plt.plot(t22_sorted['Volume'], t22_sorted['DAPI_MA'], color='red', linewidth=1, label=f'{window}-point MA')
# plt.xlabel('Volume')
# plt.xscale('log')
# plt.ylabel('DAPI Intensity')
